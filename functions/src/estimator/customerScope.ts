/**
 * Customer-facing scope for estimator → project convert.
 * Mirror of src/lib/quoteCustomerScope.ts (keep behavior aligned).
 */
...
function looksLikeInternalProjectBrief(text: string): boolean {
  const raw = text.trim();
  if (!raw) return false;
...
export function sanitizeCustomerScopeOfWork(text: string | null | undefined): string {
  ...
  const parts = raw
    ...
    .filter(Boolean);
  const cleaned = parts
    .filter((p) => !looksLikeInternalProjectBrief(p))
    .map((block) =>
      block
        .split("\n")
        .map((line) => line.trim())
