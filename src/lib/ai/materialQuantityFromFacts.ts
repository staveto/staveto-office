/**
 * Derive material quantities from projectFacts and attachment findings
 * so quote setup does not start with placeholder qty=1.
 */

import { parseAreaM2FromText } from "@/lib/ai/localizedNumber";
...
export function applyFactsToMaterialRows<T extends MaterialRowLike>(
  rows: T[],
  facts?: ProjectFactsLike | null,
  findings?: AttachmentFindingLike[] | null
): T[] {
  return rows.map((row) => {
    const hint = suggestMaterialQuantityFromFacts(
      row.name,
      row.unit,
      row.qty > 1 ? row.qty : undefined,
      facts,
      findings
    );
    if (!hint) return row;
    const isPlaceholderQty = !row.qty || row.qty <= 1;
    if (!isPlaceholderQty) return row;
    return {
      ...row,
      qty: hint.quantity,
      unit: row.unit === "pcs" && hint.unit === "m2" ? hint.unit : row.unit || hint.unit,
      sourceNote: row.sourceNote?.trim() || hint.sourceNote,
    };
  });
}
