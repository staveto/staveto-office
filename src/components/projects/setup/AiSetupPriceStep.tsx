...
export function AiSetupPriceStep({
  ...
  return (
    <div className="space-y-5">
      ...
      {pricingBlocked && pricingBlockReasons.length > 0 ? (
        <div className="rounded-xl border-2 border-amber-400 bg-amber-50 px-4 py-3 space-y-2" role="status">
          ...
          <ul className="text-sm text-amber-900 list-disc pl-5 space-y-1">
            {pricingBlockReasons.slice(0, 8).map((r, i) => (
              <li key={`${r}-${i}`}>{r}</li>
            ))}
          </ul>
      ...
      >
        {productSourcingSlot ? <div className="min-w-0 space-y-3">{productSourcingSlot}</div> : null}

        <div
          className={
            productSourcingSlot
              ? "rounded-2xl border-2 border-[#CBD5E1] bg-white p-5 sm:p-6 space-y-4 xl:sticky xl:top-4"
              : "rounded-2xl border-2 border-[#CBD5E1] bg-white p-5 sm:p-6 space-y-4"
          }
        >
          <PriceField
          ...
          <div className="border-t border-[#E2E8F0] pt-4 space-y-2">
            <Row
              label={t("projects.aiSetup.summary.margin", {
                percent: String(calculation.marginPercent),
              })}
              value={formatMoney(totals.marginAmount, currency)}
            />
            <Row
              label={t("projects.aiSetup.summary.vat", {
                percent: String(calculation.vatPercent),
              })}
          ...
          <Button
            type="button"
            className="w-full bg-[#E95F2A] hover:bg-[#D94F1F] h-11 text-base font-semibold"
            disabled={saving}
            onClick={onContinue}
          >
