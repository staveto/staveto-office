"use client";

import { Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useI18n } from "@/i18n/I18nContext";
import { cn } from "@/lib/utils";

type Props = {
  connected?: boolean;
  connectedEmail?: string;
  loading?: boolean;
  syncing?: boolean;
  onConnect?: () => void;
  onSync?: () => void;
  className?: string;
  compact?: boolean;
};

export function GmailConnectCard({
  connected,
  connectedEmail,
  loading,
  syncing,
  onConnect,
  onSync,
  className,
  compact = false,
}: Props) {
  const { t } = useI18n();

  if (connected) {
    return (
      <div
        className={cn(
          "rounded-xl border border-emerald-200 bg-emerald-50/80 p-4",
          className
        )}
      >
        <p className="text-sm font-semibold text-emerald-900">{t("inbox.connectedTitle")}</p>
        {connectedEmail ? (
          <p className="mt-1 text-sm text-emerald-800">{connectedEmail}</p>
        ) : null}
        <p className="mt-2 text-xs text-emerald-800/80">{t("inbox.autoSyncHint")}</p>
        {onSync ? (
          <Button
            type="button"
            size="sm"
            variant="outline"
            className="mt-3 border-emerald-300 bg-white"
            disabled={syncing}
            onClick={onSync}
          >
            {syncing ? <Loader2 className="mr-1.5 size-4 animate-spin" /> : null}
            {t("inbox.syncNow")}
          </Button>
        ) : null}
      </div>
    );
  }

  return (
    <div
      className={cn(
        "rounded-xl border border-[#b8c5d4] bg-white p-6 text-center shadow-sm",
        compact && "p-4",
        className
      )}
    >
      {!compact ? (
        <>
          <p className="text-lg font-bold text-[#1D376A]">{t("inbox.connectTitle")}</p>
          <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
            {t("inbox.connectDescription")}
          </p>
        </>
      ) : null}
      <Button
        type="button"
        size={compact ? "sm" : "lg"}
        disabled={loading}
        className={cn(
          "mt-4 bg-white text-[#1D376A] ring-1 ring-[#dadce0] hover:bg-[#f8f9fa]",
          !compact && "h-12 px-8 text-base font-semibold"
        )}
        onClick={() => onConnect?.()}
      >
        {loading ? (
          <Loader2 className="mr-2 size-5 animate-spin" />
        ) : (
          <svg className="mr-2 size-5" viewBox="0 0 24 24" aria-hidden>
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
        )}
        {t("inbox.connectGoogle")}
      </Button>
    </div>
  );
}
