...
function resolveImageMimeType(file: File): string | null {
  ...
  if (type && IMAGE_MIME_TYPES.has(type)) return type;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const paymentQrUrl = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(storagePath)}?alt=media&token=${downloadToken}`;
    ...
    return NextResponse.json({ ok: true, paymentQrUrl, paymentQrStoragePath: storagePath });
  } catch (e) {
    if (e instanceof AdminUnavailableError) {
      return NextResponse.json({ errorCode: "ADMIN_UNAVAILABLE" }, { status: 503 });
    }
    console.error("[organization/payment-qr]", e);
